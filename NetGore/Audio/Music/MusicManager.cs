...
namespace NetGore.Audio
{
    ...
    public class MusicManager : IMusicManager
    {
        ...
        {
            var basePath = ContentPaths.Build.Music.Join(musicInfo.Name + ContentPaths.ContentFileSuffix);
            return FindMusicFile(basePath);
        }
        
        /// <summary>
        /// Finds a music file by trying different extensions if the exact filename doesn't exist.
        /// </summary>
        /// <param name="filename">The base filename to search for</param>
        /// <returns>The actual filename with extension</returns>
        static string FindMusicFile(string filename)
        {
            // If the file exists as-is, return it
            if (System.IO.File.Exists(filename))
                return filename;
            
            // Try supported music extensions
            string[] supportedExtensions = { ".ogg", ".wav", ".flac", ".mp3" };
            ...
            // and let the Music constructor throw the appropriate error
            return filename;
        }
